...
settings.preflight = function(config, basePath, callback) {
	...
	if(config.startup_params.jar == undefined) {
	...
	if(!fs.existsSync(path.join(basePath, config.startup_params.jar))) {

			log.error(config.startup_params.jar + ".jar does not seem to be in the server directory for " + config.name);
			return;
