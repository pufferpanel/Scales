...
settings.preflight = function(config, serverPath, callback) {
	...
	async.series([
		...
		function(callback2) {
			fs.remove(path.join(serverPath, this.settings.logfile), function (error) {
...
settings.startup = function(config) {
	...
	var startupArray = config.startup.command;
	...
	if(startupArray.indexOf("-norestart") == -1) {
		startupArray += " -norestart 2>&1 | tee " + this.logfile;
	} else {
		startupArray += " 2>&1 | tee " + this.logfile;
	}
