...
settings.preflight = function(config, basePath, callback) {
	...
	if(!fs.existsSync(path.join(basePath, config.startup_params.jar + ".jar"))) {

			log.error(path.join(basePath, config.startup_params.jar + ".jar") + " does not seem to be in the server directory for " + config.name);
			return;
