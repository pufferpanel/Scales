 ...
 */
settings.preflight = function(config, callback) {

	propertiesPath = path.join(config.path, settings.cfg);

	if(!fs.existsSync(path.join(config.path, config.variables['-jar']))) {
