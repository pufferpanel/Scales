...
Plugin.prototype.test = function (t) {
    return 'hi ' + t;
};
...
Plugin.prototype.queryServer = function () {
    ...
    Gamedig.query({
        ...
        if (res.error) {
            ...
            Logger.verbose('Query error encountered for ' + this.serverConfig.gamehost + ':' + this.serverConfig.gameport, res.error);
            return false;
...
Plugin.prototype.preflight = function (next) {
    ...
    if (!fs.existsSync(Path.join(this.publicPath, this.serverConfig.startup.variables.jar))) {
        ...
        Logger.error(config.startup.variables.jar + ' does not seem to be in the server directory for ' + config.name);
        return next(config.startup.variables.jar + ' does not seem to be in the server directory.');
    }
    ...
    if (fs.existsSync(propertiesPath)) {
        try {
            ...
            Properties.parse(propertiesPath, { path: true }, function (err, obj) {
                ...
                if (err) {
                    ...
                    Logger.error('An error occured while attempting to read the server.peroperies file for ' + this.serverConfig.name, err);
                    return next('An error occured while attempting to read the server.peroperies file.');
                }
                ...
                if (obj['server-port'] !== 25565) {
                    obj['server-port'] = 25565;
                    rewrite = true;
                }

                if (obj['query.port'] !== 25565) {
                    obj['query.port'] = 25565;
                    rewrite = true;
                }
                ...
                if (rewrite) {
                    ...
                    Properties.stringify(obj, { path: propertiesPath }, function (err, object) {
                        ...
                        if (err) {
                            ...
                            Logger.error('An error occured trying to update the ' + this.settings.cfg + ' file for ' + this.serverConfig.name, err);
                            return next('An error occured trying to update the ' + this.settings.cfg + ' file');
                        }

                        log.verbose('Completed plugin preflight for ' + this.serverConfig.name);
                        next();
        ...
        log.warn('Completed plugin preflight for ' + this.serverConfig.name + ' but we need to restart the server right after because the server.properties file isn\'t generated yet.');
        next(null, 1);
    }
