...
Plugin.prototype.preflight = function (next) {
    ...
    if (typeof this.serverConfig.startup.variables.jar === 'undefined') {
        Logger.error('No server jar is defined for ' + this.serverConfig.name);
        return next(new Error('No startup jar is defined for ' + this.serverConfig.name));
    }
    ...
    if (!Fs.existsSync(Path.join(this.publicPath, this.serverConfig.startup.variables.jar))) {
        ...
        return next(new Error(this.serverConfig.startup.variables.jar + ' does not seem to be in the server directory for ' + this.serverConfig.name));
    }
