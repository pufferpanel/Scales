...
Scales.prototype.powerOn = function(callback) {
	...
	this.ps.on('data', function(data) {
		...
		s.emit('console', stripansi(output));

		// Write output to specific file if defined in the plugin
		// Used by SRCDS since it's logging is a bit.. sparse.
		if(s.plugin.manual_log != undefined && s.plugin.manual_log === true) {

			if(logStream === false) {

				fs.ensureFileSync(s.buildPath(s.plugin.log), function (error) {
					if(error) {
						log.error("An error occured while trying to create a log file for " + s.config.name, error);
					}
				});

				logStream = fs.createWriteStream(s.buildPath(s.plugin.log), {
					flags: 'a'
				});
	...
	async.series([
		...
		function(callback) {
			...
			if(build_params != undefined && build_params != "" && build_params != false) {
				var processParams = ("-b " + gconfig.basepath + " -u " + s.config.user + " " + build_params).match(/\S+/g);
			} else {
				var processParams = [ "-b", gconfig.basepath, "-u", s.config.user ];
			}

			var installerProcess = pty.spawn("./lib/scripts/" + s.plugin.install_script, processParams);

			installerProcess.on('data', function(data) {
				s.emit('installer', stripansi(data));
			});
