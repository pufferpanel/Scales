 ...
 * @return {[type]}        [description]
 */
Scales.prototype.dockerKillContainer = function (signal, crashed, next) {
    ...
    Async.series([
        ...
        function (callback) {
            ...
            dockerProcessParams = Util.format('create -it --name %s -h docker -m %sM --blkio-weight=%s --cpu-period=100000 --cpu-quota=%s %s -u %s -v %s:/home/container %s',
                ...
                self.dockerUserID,
                self.buildPath(),
                (self.config.plugin === 'bungeecord' || self.config.plugin === 'minecraft-pre') ? 'pufferpanel/minecraft' : 'pufferpanel/' + self.config.plugin
            );
...
Scales.prototype.dockerExecuteServerStartup = function (next) {
    ...
    if (typeof this.plugin.startup === 'undefined') {
        ...
        startupArray = this.plugin.startup();
    }
    ...
    try {
        ...
        this.ps = Pty.spawn('docker', startupArray.match(/\S+/g));
        return next();
    } catch (ex) {
        this.setStatus(OFF);
...
Scales.prototype.powerOff = function (next) {
    ...
    this.on('off', function () {
        Logger.warn('noticed emitted off');
        Logger.warn(typeof next);
        if (typeof next === 'function') {
            return next();
        }
    });
...
Scales.prototype.kill = function () {
    ...
    if (typeof this.ps !== 'undefined' && typeof this.ps.pid !== 'undefined') {
        this.dockerKillContainer(9, false);
        this.emit('off');
    }
};
        ...
        Usage.lookup(reference.spawnedPID, { keepHistory: true }, function (err, result) {
            ...
            if (err) {
                ...
                if (reference.statisticsErrorCount >= 3) {
                    Logger.error('There was an error attempting to perform a statistics lookup, this should not happen.', err);
                    reference.emit('crashed');
                    return;
                }
            } else {
    ...
    Async.series([
        ...
        function (callback) {
            ...
            installerProcess.on('exit', function (code) {
                ...
                if (code !== 0) {
                    return callback(new Error(Util.format('An error was encountered with the installer script for %s. Script edited with code %s and was logged to %', self.config.name, code, logLocation)));
                }
...
Scales.prototype.reinstallServer = function (plugin, build_params, next) {
    ...
    Async.series([
        function (callback) {
            ...
            if (this.status !== OFF) {
                self.dockerKillContainer(9, false, function (err) {
        ...
        function (callback) {
            ...
            if (plugin !== null) {
                ...
                Fs.writeFile('./data/' + self.config.name + '.json', JSON.stringify(self.config, null, 4), function (err) {
