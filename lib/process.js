...
Scales.prototype.install = function(hash, password) {

	var downloaded = false;
	fs.ensureDir("./data/downloads", function(err) {
		if(err != null) {
			console.log(err);
		}
	});
	...
	// Add the User
	proc.exec("./scripts/create_user.sh " + gconfig.basepath + " " + this.config.user + " " + password, function(error, stdout, stderr) {

		if(error !== null) {
			log.error("Error occured trying to execute a command to add a user.", error);
		}

	});

	// Download File from PufferPanel Remote Repository
	if(hash !== null && !fs.existsSync('./data/downloads/' + hash + '.tar.gz')) {
	...
	// Unpack file to server; skip doing this if there was an error
	if(downloaded !== true) {
		return;
	}

	// Handle Moving Files and Such
