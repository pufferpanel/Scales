    ...
    Async.series([
        ...
        function (callback) {
            ...
            var portMap = Util.format('-p %s:%s:%s -p %s:%s:%s/udp',
                ...
                self.plugin.settings.ports.listen,
                self.config.gamehost,
                self.config.gameport,
                self.plugin.settings.ports.listen
            );
...
Scales.prototype.majorInstall = function (buildParams, next) {
    ...
    Async.series([
        ...
        if (err && err !== 'no script') {
            ...
            Logger.error('Unable to complete the installer process. Hanging server until this is looked at...', err);
...
Scales.prototype.reinstallServer = function (plugin, build_params, next) {
    ...
    Async.series([
        function (callback) {
            ...
            if (this.status !== OFF) {
                self.dockerKillContainer(function (err) {
