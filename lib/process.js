...
Scales.prototype.powerOn = function(callback) {
	...
	try {
		...
		var startupArray = this.config.startup;
		for(var index in this.config.startup_params) {

			var startupArray = startupArray.replace("${" + index + "}", this.config.startup_params[index]);
...
Scales.prototype.mergeJson = function(json, obj, overwrite, res) {
	...
	if(obj !== null) {
		if(overwrite !== true) {
			finalObject[obj] = extend({}, this.config[obj], JSON.parse(json));
		} else {
			finalObject[obj] = JSON.parse(json);
		}
	} else {
		if(overwrite !== true) {
	...
	fs.writeFile("./data/" + this.config.name + ".json", JSON.stringify(finalObject, null, 4), function(error) {
		...
		if(error) {
			...
			log.verbose("Config for server has been updated.");
