...
Scales.prototype.preflight = function() {
	...
	async.series([
		function(callback) {
			proc.exec("chown -R " + s.config.user + ":scalesuser " + this.buildPath(null), function(error, stdout, stdin) {
...
Scales.prototype.install = function(hash, password) {
	...
	async.series([
		...
		function(callback) {
			...
			// Add the User
			log.verbose("Creating user " + self.config.user + " on the server...");
			proc.exec("./lib/scripts/create_user.sh " + gconfig.basepath + " " + self.config.user + " " + password, function(error, stdout, stderr) {

				if(error !== null) {
					log.error("Error occured trying to execute a command to add a user.", stderr);
				} else {
					log.verbose("User " + self.config.user + " created.");
					callback();
				}
		...
		function(callback) {
			...
			try {
				...
				fs.copy('./data/downloads/unpack/' + hash, self.buildPath(null), function(error){
					if(error) {
