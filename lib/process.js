...
var Scales = function (config) {
    ...
    this.controller = new SystemControl(GlobalConfig.useDocker | true, this);
...
Scales.prototype._dockerGetUserID = function (next) {
...
Scales.prototype._initialize = function (next) {
    ...
    var self = this;
    self.controller.initialize(self.config.user, next);
};
...
Scales.prototype._stop = function (next) {
    ...
    var self = this;
    self.controller.stop(self.config.user, next);
};
...
Scales.prototype._kill = function (next, crashed) {
    ...
    self.controller.kill(self.config.user, next);
};
...
Scales.prototype._delete = function (next) {
    ...
    var self = this;
    self.controller.delete(self.config.user, next);
};
...
Scales.prototype.buildPath = function (extended) {
    ...
    if (publicPath.indexOf(Path.join(GlobalConfig.basepath, this.config.user, 'public')) !== 0) {
        Logger.warn('API attempted to access a file outside of base directory ' + GlobalConfig.basepath + this.config.user);
