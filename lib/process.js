 ...
 */
Scales.prototype.getRootPath = function () {
    return Path.join(gconfig.basepath, this.config.name);
};
...
Scales.prototype.buildPath = function (extended) {
    ...
    if (publicPath.indexOf(gconfig.basepath + this.config.user) !== 0) {
        ...
        Logger.error('API attempted to access a file outside of base directory ' + gconfig.basepath + this.config.user + '. Request denied.');
...
Scales.prototype.install = function (res, hash, password, build_params) {
    ...
    Async.series([
        ...
        function (next) {
            ...
            proc.exec('./lib/scripts/create_user.sh ' + gconfig.basepath + ' ' + self.config.user + ' ' + password, function (err, stdout, stderr) {
