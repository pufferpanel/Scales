...
var Scales = function (config) {
    ...
    this.controller = new SystemControl(((typeof GlobalConfig.useDocker === 'undefined') || (GlobalConfig.useDocker) == true), this);
...
Scales.prototype.majorInstall = function (buildParams, next) {
    ...
    Async.series([
        ...
        function (callback) {
            ...
            Logger.verbose(Util.format('Executing %s %s', './lib/scripts/' + self.plugin.settings.install_script, processParams));

            var installerProcess = Pty.spawn();
