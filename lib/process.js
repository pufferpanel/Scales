    ...
    this.dockerInitalizeContainer(function (err) {
        ...
        self.ps.on('data', function (data) {
            ...
            if (self.spawnedPIDLocked === false && self.ps.pid && typeof self.spawnedPID === 'undefined') {
                ...
                Proc.exec(Util.format('pgrep -u %s %s', self.config.user, self.plugin.settings.exe), function (err, stdout, stderr) {
...
Scales.prototype.statistics = function (reference) {
    ...
    if (reference.spawnedPID) {
        Usage.lookup(reference.spawnedPID, { keepHistory: true }, function (err, result) {
            ...
            if (err) {
                Logger.error('There was an error attempting to perform a statistics lookup.', err);
                return;
            }
    ...
    Async.series([
        ...
        function (next) {
            ...
            dockerProcessParams = [
                ...
                '-p',
                self.config.gamehost + ':' + self.config.gameport + ':' + (self.plugin.settings.ports.listen || 8080) + '/udp',
                '-m',
                ...
                (self.config.plugin === 'bungeecord') ? 'pufferpanel/minecraft' : 'pufferpanel/' + self.config.plugin
            ];
        ...
        function (next) {
            ...
            if (typeof buildParams !== undefined && buildParams !== '' && buildParams) {
                processParams = Util.format('-b %s -u %s %s', Config.global.basepath, self.config.user, buildParams);
            } else {
                processParams = Util.format('-b %s -u %s', Config.global.basepath, self.config.user);
            }

            if (self.config.plugin === 'srcds') {
                processParams = Util.format('-d %s ', self.config.user) + processParams;
            }
