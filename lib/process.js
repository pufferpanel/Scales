...
Scales.prototype.preflight = function () {
    ...
    Async.series({
        preflight: function (next) {
            ...
            Logger.verbose('Executing plugin preflight');
        ...
        startup: function (next) {
            ...
            Logger.verbose('Powering on server');
            self.powerOn(next);
        }
...
Scales.prototype.writeFile = function (base, contents) {
    ...
    try {
        ...
        Fs.chownSync(filepath, Userid.uid(this.config.user), Userid.gid(this.config.user));
        return true;
    } catch (ex) {
    ...
    Async.series([
        ...
        function (callback) {
            ...
            var params = [
                ...
                self.config.user,
                '-c',
                __dirname + '/scripts/' + self.plugin.settings.install_script + ' ' + processParams
            ];

            //var cmd = './lib/scripts/' + self.plugin.settings.install_script;
            Logger.verbose(Util.format('Executing %s %s', cmd, params.join(' ')));

            var installerProcess = Pty.spawn(cmd, params);
            ...
            installerProcess.on('exit', function (code) {
                ...
                Logger.verbose('Updating root folder perms');
                try {
                    var result = Proc.execSync(Util.format('chown root:root %s/%s', GlobalConfig.basepath, self.config.user));
                    Logger.verbose(result);
                    result = Proc.execSync(Util.format('chmod 755 %s/%s', GlobalConfig.basepath, self.config.user));
                    Logger.verbose(result);
                } catch (err) {
