...
Scales.prototype.install = function (res, hash, password, build_params) {
    ...
    Async.series([
        ...
        function (next) {
            ...
            Logger.verbose(Util.format('Creating docker container for %s', self.config.name));
            proc.exec(Util.format('docker create -it --name %s -h docker -p %s:%d:%d -m %dM -c %d --blkio-weight=%d -u `stat -c \'%u:%g\' %s` -v %s:/data pufferpanel/%s',
                self.config.user,
                ...
                if (err) {
                    ...
                    return next(stderr, 'Error occured trying to execute command to add a docker container.');
                }

                Logger.verbose(Util.format('Successfully added a new docker container for %s identified by %s', self.config.name, stdout));
                return next();
