...
Scales.prototype.install = function(res, hash, password, srcds) {
...
Scales.prototype.majorInstall = function(srcds) {
	...
	async.series([
		...
		function(callback) {
			...
			proc.exec("./lib/scripts/" + s.plugin.install_script + " " + s.config.user + " " + srcds, function(error, stdout, stderr) {

				if(error !== null) {
					log.error("Error occured trying to execute installer command.", stderr);
					return callback("Error occured trying to execute installer command.");
				} else {
