    ...
    if (self.isDocker) {
        Async.series([
            function (callback) {
                ...
                parent._dockerGetUserID(function (err) {
            ...
            function (callback) {
                ...
                var dockerProcessParams = Util.format('create -it --name %s -h docker -m %sM --blkio-weight=%s --cpu-period=100000 --cpu-quota=%s %s -u %s -v %s:/home/container %s',
                    ...
                    parent.dockerUserID,
                    parent.buildPath(),
                    (self.config.plugin === 'bungeecord' || self.config.plugin === 'minecraft-pre') ? 'pufferpanel/minecraft:latest' : 'pufferpanel/' + parent.config.plugin + ':latest'
                );
...
SystemControl.prototype.start = function () {
