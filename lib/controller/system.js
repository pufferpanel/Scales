...
SystemControl.prototype.start = function (next) {
    ...
    if (self.isDocker) {
        try {
            ...
            var cmdParams = Util.format('exec -it %s %s', self.parent.plugin.serverConfig.user, self.parent.plugin.startup());
            Logger.verbose('Executing: docker ' + cmdParams);
            self.parent.ps = Pty.spawn('docker', cmdParams.match(/\S+/g));
            return next();
        } catch (ex) {
