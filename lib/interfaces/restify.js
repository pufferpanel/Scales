...
server.on('uncaughtException', function (req, res, route, err) {
    log.error('Uncaught Exception in resitfy function, unable to process request.', err.stack);
...
server.get('/server/:id/power/:action', function(req, res, next) {
	...
	if(req.params.action == "on") {
		s[req.params.id].preflight();
	} else if(req.params.action == "off") {
		s[req.params.id].powerOff();
	} else if(req.params.action == "restart") {
		s[req.params.id].powerCycle();
	} else if(req.params.action == "kill") {
		s[req.params.id].kill();
	}
